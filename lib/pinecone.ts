...
import { downloadFromAzureBlobStorage } from "./azure-server";
...
//   const file_name = await downloadFromS3(fileKey);
  const file_name = await downloadFromAzureBlobStorage(fileKey);
  if (!file_name) {
  ...
  documents.forEach((document)=>{
    ...
    content=content.concat(document[0].pageContent)
  })

  const docu1 : Document={
    pageContent:content,
    metadata: {
      pageNumber: 1,
